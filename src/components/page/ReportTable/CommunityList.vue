...
//          监听搜索框
        ...
        methods: {
            ...
            download(){
                ...
                sels = sels.join(",");
                if(sels){
                    window.location.href=`http://cactus.thextrader.cn/api/V1/lzCloud/group/export?uid=${userCurrent.id}&rid=${userCurrent.roleId}&sels=${sels}&startTime=${this.date_range[0].split("-").join("")}&endTime=${this.date_range[1].split("-").join("")}`;
